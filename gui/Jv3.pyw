...
class January:
    ...
    def januaryActivatedUI(self):
        Activate = open("memory\logs\InSyncWithActivationFile.txt", "w");
        Activate.write("sync\n");
        Activate.write("1");
        Activate.close();
        os.startfile('Activation.pyw');
    def januaryDeactivatedUI(self):
        Activate = open("memory\logs\InSyncWithActivationFile.txt", "w");
        Activate.write("sync\n");
        Activate.write("0");
        Activate.close();
    ...
    def farewell(self):
        ...
        ShortReminder = open("memory\Reminders\ShortReminder.txt", "w");
        ShortReminder.write("reminder,hour,min\n");
        ShortReminder.write("0,0,0");
        ShortReminder.close();
    ...
    def programLauncher(self):
        ...
        elif "alarm" in SpokenWords:
            j.programAlarmApp();
        else :
            j.AI_Ok();
            os.system('explorer D:\\"{}"'.format(SpokenWords.replace('open ', '').replace('launch ', '')));
    ...
    def programAlarmApp(self):
        playsound("Voice\Reminders\ActivatingAlarm.mp3");
        os.startfile('JanuaryAlarm.pyw');
    def AI_ShortReminder(self):
        playsound("Voice/Reminders/ReminderWhat.mp3");
        while 1:
            j.actionListener();
            if Unrecognized == 1:
                reminderTopic = SpokenWords;
                break;
        hour = 0; minute = 0;
        if "hours" in SpokenWords:
            BagOfWords = SpokenWords.split();
            target = "hours";
            EndPoint = len(BagOfWords);
            TargetPoint = BagOfWords.index(target) + 1;
            if TargetPoint <= EndPoint and TargetPoint > 1:
                hour = BagOfWords[BagOfWords.index(target) - 1];
        elif "hour" in SpokenWords:
            BagOfWords = SpokenWords.split();
            target = "hour";
            EndPoint = len(BagOfWords);
            TargetPoint = BagOfWords.index(target) + 1;
            if TargetPoint <= EndPoint and TargetPoint > 1:
                hour = BagOfWords[BagOfWords.index(target) - 1];
                if "an" in SpokenWords:
                    hour = 1;
        if "minutes" in SpokenWords:
            BagOfWords2 = SpokenWords.split();
            target = "minutes";
            EndPoint = len(BagOfWords2);
            TargetPoint = BagOfWords2.index(target) + 1;
            if TargetPoint <= EndPoint and TargetPoint > 1:
                minute = BagOfWords2[BagOfWords2.index(target) - 1];
        elif "minute" in SpokenWords:
            BagOfWords2 = SpokenWords.split();
            target = "minute";
            EndPoint = len(BagOfWords2);
            TargetPoint = BagOfWords2.index(target) + 1;
            if TargetPoint <= EndPoint and TargetPoint > 1:
                ...
                if "a" in SpokenWords:
                    minute = 1;
        currentTime = datetime.datetime.now();
        TargetHour = int(hour) + int(currentTime.hour);
        TargetMin = int(minute) + int(currentTime.minute);
        if TargetHour >= 24:
        ...
        ShortReminder = open("memory\Reminders\ShortReminder.txt", "w");
        ShortReminder.write("reminder,hour,min\n");
        ShortReminder.write(reminderTopic+","+str(TargetHour)+","+str(TargetMin));
        ShortReminder.close();
        playsound("Voice/Reminders/ReminderWillRemind.mp3");
    def AI_Quotes(self):
    ...
    def commentOrCompliment(self):
        ...
        elif "interesting" in SpokenWords or "fascinating" in SpokenWords:
            from random import randint;
            number = randint(0, 3);
            if number == 0:
    ...
    try:
        ...
        if "hi " in SpokenWords or "hello" in SpokenWords or "hey" in SpokenWords:
            ...
            if "good" in SpokenWords:
                ...
                j.commentOrCompliment();
            elif "set" in SpokenWords:
                if "alarm" in SpokenWords:
                    j.programAlarmApp();
            elif "need a reminder"in SpokenWords or "use a reminder" in SpokenWords or "need a short reminder"in SpokenWords:
                j.AI_ShortReminder();
